...
int main(int argc, char** argv)
{
  ...
  int dsNum=-1;
  bool loadTrees=0, useDB=0, docs=0;
  ...
  for (size_t i = 0; i < opt.size(); i++) {
    ...
    if (opt[i] == "-v") { noVetoRuns(dsNum); return 0;}
  ...
  checkData(dsNum,loadTrees,runList);
}
...
void noVetoRuns(int dsNum)
{
  ...
  for (auto run : runList)
  {
    GATDataSet dsRun(run);
    string vetPath = dsRun.GetPathToRun(run,GATDataSet::kVeto);
    TFile *vetFile = new TFile(vetPath.c_str());

    // Check file exists
    if (vetFile->IsZombie()) {
      cout << Form("Run %i has no veto data.\n",run);
      noVeto.push_back(run);
      delete vetFile;
      continue;
    }
    // Check we have nonzero entries
    TTree *vet = (TTree*)vetFile->Get("vetoTree");
    long nVet = vet->GetEntries();
    if (nVet == 0) {
      cout << Form("Run %li, zero entries in veto chain.\n",nVet);
      noVeto.push_back(run);
    }
    delete vetFile;
  }

  // Now print the no-veto runs kind of compactly
  for (auto run : noVeto) cout << run << " ";
  cout << endl;
